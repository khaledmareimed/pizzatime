...
export async function POST(request: NextRequest) {
  try {
    ...
    // Track material usage for this order
    try {
      await trackMaterialUsageForOrder(newOrder, session.user.id || session.user.email)
    } catch (error) {
      console.error('❌ Error tracking material usage for order:', error)
      // Don't fail the order creation if material tracking fails
    }
...
/**
 * Track material usage when an order is created
 * This function estimates material consumption based on ordered products
 */
async function trackMaterialUsageForOrder(order: any, userId: string) {
  try {
    // Get material collection
    const materialCollection = await createCollection<RawMaterial>('materials', RawMaterialSchema, {
      indexes: RawMaterialIndexes
    })

    // Get Jordan timezone date
    const jordanTime = new Date()
    const jordanOffset = 3 * 60 // Jordan is UTC+3
    const localOffset = jordanTime.getTimezoneOffset()
    const jordanOrderTime = new Date(jordanTime.getTime() + (localOffset + jordanOffset) * 60000)

    // Process each item in the order
    for (const item of order.items) {
      try {
        // For now, we'll use a simple estimation: 1 unit of material per product
        // In a real system, you would have a recipe/BOM (Bill of Materials) system
        // that defines exactly which materials and quantities are needed for each product
        
        // Find materials that might be used for this product category
        const materials = await materialCollection.model.find({
          status: 'active',
          // You could add category matching here if you have product-material relationships
          // category: getRelatedMaterialCategory(item.categoryId)
        }).limit(5) // Limit to avoid processing too many materials

        // For demonstration, we'll assume each product uses a small amount of the first few materials
        for (const material of materials) {
          const estimatedUsage = item.quantity * 0.1 // Estimate 0.1 units of material per product

          if (estimatedUsage > 0 && material.currentStock >= estimatedUsage) {
            // Add usage record
            const usageRecord = {
              quantity: estimatedUsage,
              usageDate: jordanOrderTime,
              purpose: 'Production',
              notes: `استهلاك تلقائي من الطلب #${order.orderId} - ${item.productName} (${item.quantity} قطعة)`,
              usedBy: userId,
              createdAt: jordanOrderTime,
              metadata: {
            ...
            // Add usage to material
            material.usages.push(usageRecord)
            
            // Update current stock
            material.currentStock -= estimatedUsage
            material.updatedAt = jordanOrderTime
            material.updatedBy = userId

            await material.save()

            console.log(`✅ Tracked material usage: ${material.name} - ${estimatedUsage} ${material.unit} for order ${order.orderId}`)
          }
        }

      } catch (itemError) {
        console.error(`❌ Error processing material usage for item ${item.productName}:`, itemError)
        // Continue with other items even if one fails
      }
    }

    console.log(`✅ Material usage tracking completed for order ${order.orderId}`)

  } catch (error) {
    console.error('❌ Error in trackMaterialUsageForOrder:', error)
    throw error
  }
}